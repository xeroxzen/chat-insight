...
def generate_visualizations(df: pd.DataFrame, config: VisualizationConfig, chat_type='friends') -> Dict[str, Path]:
    ...
    try:
        ...
        if chat_type == 'friends':
            # Group-specific visualizations are now handled by generate_group_visualizations
            # This avoids duplication and ensures consistent paths
            logger.info("Group-specific visualizations will be generated by generate_group_visualizations")
        else:
...
class ChatAnalysis:
    ...
    def to_dict(self) -> dict:
        ...
        logger.info("Converting ChatAnalysis to dictionary")
        
        # Check if visualization_paths contains group-specific visualizations
        if self.is_group:
            logger.info("This is a group chat, checking for group-specific visualizations")
            if "group_participation" in self.visualization_paths:
                logger.info(f"group_participation found: {self.visualization_paths['group_participation']}")
            else:
                logger.error("group_participation is missing from visualization_paths!")
                
            if "group_interaction_network" in self.visualization_paths:
                logger.info(f"group_interaction_network found: {self.visualization_paths['group_interaction_network']}")
            else:
        ...
        # Log the visualization paths in the result
        logger.info("Visualization paths in result:")
...
def generate_group_visualizations(df: pd.DataFrame, config: VisualizationConfig) -> Dict[str, Path]:
    ...
    try:
        logger.info("Generating group-specific visualizations")
        ...
        logger.info(f"Generated group_participation visualization: {participation_path}")
        ...
        logger.info(f"Generated group_interaction_network visualization: {network_path}")
        
        result = {
            "group_participation": participation_path,
            "group_interaction_network": network_path
        }
        
        logger.info(f"Returning group visualizations: {result}")
        return result
    except Exception as e:
        logger.error(f"Error generating group visualizations: {str(e)}")
        return {}
...
def analyze_chat_log(csv_file_path: str, output_dir: Optional[Path] = None) -> dict:
    ...
    try:
        ...
        if is_group:
            logger.info("This is a group chat, generating group-specific visualizations")
            ...
            logger.info("Calling generate_group_visualizations")
            group_viz = generate_group_visualizations(df, config)
            
            # Log the group visualization paths
            logger.info("Group visualization paths from generate_group_visualizations:")
            for key, path in group_viz.items():
                logger.info(f"  {key}: {path}")
                
            # Update visualization_paths with group-specific visualizations
            visualization_paths.update(group_viz)
            
            # Update analysis.visualization_paths with the updated visualization_paths
            analysis.visualization_paths.update(group_viz)
            
            # Double-check that the paths were added
            logger.info("After update, analysis.visualization_paths contains:")
            if 'group_participation' in analysis.visualization_paths:
                logger.info(f"  group_participation: {analysis.visualization_paths['group_participation']}")
            else:
                logger.error("  group_participation is missing from analysis.visualization_paths!")
                
            if 'group_interaction_network' in analysis.visualization_paths:
                logger.info(f"  group_interaction_network: {analysis.visualization_paths['group_interaction_network']}")
            else:
                logger.error("  group_interaction_network is missing from analysis.visualization_paths!")
                
            # Log all visualization paths
            logger.info("All visualization paths in analysis.visualization_paths:")
            for key, path in analysis.visualization_paths.items():
                logger.info(f"  {key}: {path}")
        else:
            logger.info("This is an individual chat, generating individual-specific visualizations")
            ...
            # Update analysis.visualization_paths with the updated visualization_paths
            analysis.visualization_paths.update(individual_viz)
        ...
        # Update analysis.visualization_paths with the heatmap
        analysis.visualization_paths['heatmap'] = heatmap_path
