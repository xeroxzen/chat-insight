...
def mock_dataframe():
    ...
    # Creating a DataFrame with 30 different senders to test top 20 filtering
    senders = [f"User{i}" for i in range(1, 31)]
    
    # Creating sample data with varying message counts per sender
    ...
    # Creating messages for each sender with different counts to ensure ranking
    for i, sender in enumerate(senders):
        ...
        # Generating sample messages for each sender
        for j in range(message_count):
            # Creating messages across different dates and times
            message_date = base_date - timedelta(days=j % 30)
            message_time = f"{(j % 24):02d}:{(j % 60):02d}:{(j % 60):02d}"
            
            # Adding some emojis to messages
            emojis = ""
            if j % 5 == 0:
                emojis = "😊"
            elif j % 7 == 0:
    ...
    # Adding MessageLength column
    df["MessageLength"] = df["Message"].str.len()
    
    # Adding Sentiment and Sentiment_Label columns required by generate_visualizations
...
class TestVisualizationLimits:
    ...
    def setup(self, monkeypatch):
        ...
        # Creating a directory for test output
        os.makedirs("test_output", exist_ok=True)
        
        # Patching the STOPWORDS in WordCloud to avoid issues
        monkeypatch.setattr("analyzer.STOPWORDS", set())
        
        # Patching the re.findall function to handle emoji pattern
        original_findall = re.findall
        def patched_findall(pattern, string):
            if pattern == analyzer.EMOJI_PATTERN:
                # Returning some emojis for testing
                if "😊" in string:
                    return ["😊"]
                elif "👍" in string:
                    return ["👍"]
    ...
    def test_message_length_distribution_limit(self, mock_save, mock_sns, mock_plt, mock_wordcloud, mock_dataframe, mock_config):
        ...
        with patch('analyzer.generate_visualizations') as mock_generate:
            # Setting up the mock to return a dictionary with the expected keys
            ...
            # Calling the function with 'friends' chat type (group chat)
            visualizations = mock_generate(mock_dataframe, mock_config, chat_type='friends')
            
            # Checking that the function was called with the right arguments
            mock_generate.assert_called_once_with(mock_dataframe, mock_config, chat_type='friends')
            
            # Verifying that the expected keys are in the result
            assert 'message_length_distribution' in visualizations
            
            # Verifying that the top 20 senders were selected
    ...
    def test_individual_chat_no_limit(self, mock_save, mock_plt, mock_wordcloud, mock_dataframe, mock_config):
        ...
        # Creating a smaller dataframe with just 2 senders for individual chat
        ...
        with patch('analyzer.generate_visualizations') as mock_generate:
            ...
            # Calling the function with 'romantic' chat type (individual chat)
            ...
            # Checking that visualizations were created
            assert 'message_length_distribution' in visualizations
            assert 'links_shared_by_sender' in visualizations
            assert 'messages_count_per_sender' in visualizations
            assert 'daily_pattern' in visualizations
...
class TestEmojiAnalysis:
    ...
    def test_emoji_usage_top5_limit(self, mock_dataframe):
        ...
        # Adding emoji data to the dataframe
        ...
        # Checking that emoji stats are limited to top 5 participants
        assert 'emoji_stats_by_sender' in emoji_analysis
        assert len(emoji_analysis['emoji_stats_by_sender']) <= 5
        
        # Verifying that the top 5 senders were selected (they have the most messages)
